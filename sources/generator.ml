...
let generate (type elt) (iterator : (elt, 'container) iterator)
    (container : 'container) : elt generator =
  let open Effect in
  ...
  let rec gen =
    ref (fun () ->
        match_with
          (iterator (fun elt -> perform (Yield elt)))
          container
          {
            retc =
              (fun () ->
...
let inf : 'a generator -> 'a stream =
 fun g () -> match g () with Some n -> n | _ -> assert false
...
let rec filter : 'a stream -> ('a -> bool) -> 'a stream =
 fun g p () ->
  let v = g () in
  if p v then v else filter g p ()

(* map stream *)
let rec map : 'a stream -> ('a -> 'b) -> 'b stream = fun g f () -> f (g ())
